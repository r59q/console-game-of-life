...
use crate::{
    components::{controllable::Controllable, position::Position, velocity::Velocity},
    input_manager::axis::Axis,
    resources::{inputs::axis_inputs::AxisInputs, pause_state::PauseState, timer::Timer},
    systems::{axis_velocity::axis_velocity, movement::movement_system, timing::timing_system},
    tests::initialize_with_entity_and_timing_system,
};
...
fn can_add_movement_system() {
    ...
    schedule.add_stage(
        "update",
        SystemStage::parallel().with_system(movement_system),
    );
}
...
fn different_velocities_makes_different_changes() {
    ...
    let entity1 = test_env
        ...
        .id();
    let entity2 = test_env
        .game
        .get_world_mut()
        .spawn()
        .insert(Position { x: 0., y: 0. })
        .insert(Velocity { x: 2., y: 2. })
        .id();
...
fn can_add_axis_velocity_system() {
    ...
    test_env
        .game
        .get_world_mut()
        .insert_resource(AxisInputs::new());
    test_env
        .game
        .get_world_mut()
        .insert_resource(PauseState::new());
    ...
    test_env.game.add_stage_to_schedule(
        "preupdate",
        SystemStage::parallel().with_system(axis_velocity),
    );
    test_env.game.add_stage_to_schedule(
        "update",
        SystemStage::parallel().with_system(movement_system),
...
fn can_pause_movement() {
    ...
    test_env
        .game
        .get_world_mut()
        .insert_resource(pause_resource);
    test_env
        .game
        .add_stage_to_schedule("move", SystemStage::parallel().with_system(movement_system));

    test_env.game.run_schedule();

    let moved_position1 = test_env
        .game
        .get_world_ref()
