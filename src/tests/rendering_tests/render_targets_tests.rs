...
use crate::{
    ...
    systems::drag_view_offset::drag_view_offset,
    tests::initialize,
};
...
fn mouse_inputs_changes_offset() {
    ...
    mouse_inputs.set_state(MouseButton::Right, InputAction::Down, 0, 0);
    mouse_inputs.set_state(MouseButton::Right, InputAction::Drag, 1, 1);

    test_env
        .game
        .get_world_mut()
        .insert_resource::<ViewOffset>(view_offset);
    test_env
        .game
        .get_world_mut()
        .insert_resource::<MouseInputs>(mouse_inputs);

    test_env.game.add_stage_to_schedule(
        "move",
        SystemStage::single_threaded().with_system(drag_view_offset),
    );

    test_env.game.run_schedule();

    let drag_delta = test_env
        .game
        .get_world_mut()
    ...
    let fetched_offset = test_env
        .game
        .get_world_ref()
        .get_resource::<ViewOffset>()
        .unwrap();
...
#[test]
fn left_mouse_button_does_not_change_offset() {
    let mut test_env = initialize();
    ...
    assert_eq!(fetched_offset.get_offset(), (0, 0))
}
...
fn mouse_inputs_changes_iteratively_offset() {
    ...
    test_env
        ...
        .get_resource_mut::<MouseInputs>()
        .unwrap()
        .set_state(MouseButton::Right, InputAction::Drag, 2, 2);

    let drag_delta_new = test_env
        .game
        .get_world_mut()
        .get_resource_mut::<MouseInputs>()
        .unwrap()
